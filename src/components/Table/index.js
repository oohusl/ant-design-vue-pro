  ...
  methods: {
    ...
    loadData (pagination, filters, sorter) {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          that.localPagination = that.showPagination && Object.assign({}, that.localPagination, {
            current: (pagination && pagination.current) || that.showPagination && that.localPagination.current || that.pageNum,
            total: r['x-total-count'], // 返回结果中的总记录数
            pageSize: (pagination && pagination.pageSize) ||
              that.localPagination.pageSize
          }) || false
          // 为防止删除数据后导致页面当前页面数据长度为 0 ,自动翻页到上一页
          if (r.length === 0 && that.showPagination && that.localPagination.current > 1) {
          ...
          that.localDataSource = r // 返回结果中的数组数据
